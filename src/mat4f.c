...
void mat4f_make_orthographic(mat4f_t* m, float l, float r, float t, float b, float n, float f) {
	m->data[0][0] = 2.0f / (r - l);
	m->data[0][1] = 0.0f;
	m->data[0][2] = 0.0f;
	m->data[0][3] = 0.0f;
	...
	m->data[3][0] = 0.0f;  // -(r + l) / (r - l);
	m->data[3][1] = 0.0f;  // -(t + b) / (t - b);
	m->data[3][2] = 0.0f;  // -(f + n) / (f - n);
	m->data[3][3] = 1.0f;
}
